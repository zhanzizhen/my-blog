部分摘自 [mdn](https://link.zhihu.com/?target=https%3A//developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Regular_Expressions%23special-non-capturing-parentheses)
...
- `(?<=y)x`: 匹配x，且这个x的前面**有**个y。
- `x(?=y)`：匹配x，且这个x的后面**有**个y。
- `(?：匹配x，且这个x的前端**没有**y。
- `x(?!y)`：匹配x，且这个x的后面**没有**y。

## 记住和不记住这次匹配

- `(x)`：正则表达式中，()的功能是划分组并且用序号记住这个组，如：

```js
/(me)\1/;   // 相当于/meme/
/(me)(you)(he)\1\3/ ;   // 相当于/meyouhemehe/
```

这在写一些复杂正则的时候特别有用，让正则更加可读，也更好维护。 以上是作用域正则表达式的”匹配环节“，其实在正则表达式的替换环节，也可以使用$n的语法来引用匹配环节中已经记住的项：

```js
'hello'.replace(/(hello)/, '$1 world') // 'hello world'
// 这里hello被划分成组1，$1引用了这个组1
```
...
```js
/(me)(?:you)(he)\1\3/ ;   
// 错误的正则，因为第二个的(?:you)没被记住，(he)的代号变成了\2，\3会匹配失败
```

分组的功能很好用，可以和match搭配使用。比如我们想匹配前面有hello的world字符串，除了用前瞻，还可以：

```js
'hello world'.match(/hello (world)/)[1]   // world
```

当有全局匹配符g时，分组功能会失效

---
String.prototype.replace还支持function (replacement){}作为第二个参数